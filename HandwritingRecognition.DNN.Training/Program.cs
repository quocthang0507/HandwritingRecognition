...
namespace HandwritingRecognition.DNN.Traning
{
	class Program
	{
		private static string outputModelPath { get; set; }
		private static string inputTrainingFolderPath { get; set; }
		private static string inputTestFolderPath { get; set; }
		...
		{
			trainingDataset = TransformImagesToIDataView(inputTrainingFolderPath);
			DataOperationsCatalog.TrainTestData validationTestSplit = mlContext.Data.TrainTestSplit(TransformImagesToIDataView(inputTestFolderPath), 0.3);
		...
		{
			ImageClassificationTrainer.Options options = new()
			{
